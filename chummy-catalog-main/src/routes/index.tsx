...
function Index() {
  const featured = products.filter((p) => p.tags.includes("destacado")).slice(0, 3);
  const bestSellers = products.filter((p) => p.tags.includes("mas-vendido")).slice(0, 3);
  const offers = products.filter((p) => p.oldPrice).slice(0, 3);
